...
public class TesteRepositorioIndice {
	...
	static void indexar(){
		System.out.println("Iniciando coleta e indexação!!!");
		long inicio = System.currentTimeMillis();
		try {
			
			IndiceControle controle = MTDFactory.getInstancia().newControleIndice();
			//TODO: obter dados dos repositorios a indexar (URL, metadataPrefix, set)
			//de arquivo properties
			
			//controle.indexar("http://tede.pucrs.br/tde_oai/oai3.php", "mtd2-br",null);
			controle.indexar("http://www.repositorio.ufpe.br/oai/request", "qdc","com_123456789_50");
			
		} catch (Exception e) {
			//enviar email de alert para equipe tecnica se servico der excecao
			e.printStackTrace();
